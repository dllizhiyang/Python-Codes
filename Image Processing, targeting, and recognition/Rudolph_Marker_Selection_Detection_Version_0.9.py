...
class select_and_find:
    ...
    def Template_Matching_edge_to_edge(self):
        input_img = cv2.imread('%s'%circuit_path,0)
        gauss_blurred_img = cv2.GaussianBlur(input_img, (7, 7), 0)
        edged_img = cv2.Canny(gauss_blurred_img,50, 150)
        image_height = gauss_blurred_img.shape[0]
        image_wide = gauss_blurred_img.shape[1]
        img0 =cv2.imread('%s'%circuit_path)
        img2 = input_img.copy()
        template = cv2.imread('%s_section1_edge.jpg'%template_path,0)
        template_shown = cv2.imread('%s'%template_path)
        print("%s,%s"%(image_height,image_wide))
        h = template.shape[0]
        w = template.shape[1]
...
def on_mouse(event, x, y, flags,param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        cv2.imwrite('%s_section1_blurred.jpg'%template_path, blurred)
        cv2.imshow("blurred",blurred)
        edged = cv2.Canny(cut_img, 50, 500)
        cv2.imwrite('%s_section1_edge.jpg'%template_path,edged)
...
def main():
    ...
    circuit_path = 'real_circuit_board.jpg'
    template_path = 'real_circuit_board_section1.jpg'
    #circuit_path = 'iphone_pcb.jpg'
    #template_path = 'iphone_pcb_section1.jpg'
    global img
    img = cv2.imread('%s'%circuit_path)
    print(time.time())
