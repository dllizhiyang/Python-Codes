...
class find_Square_Marker:
	...
	def find_Square(self,rawImage):
		...
		for i in contours:
			...
			approx = cv2.approxPolyDP(i,0.01*epsilon,True)
			#print(approx)
			if len(approx)>=4 and len(approx)<=6:  ########  rectangle has roughly 5~6 contours########
				(x,y,w,h) = cv2.boundingRect(approx)
				ratio = w/h
				good_ratio = (ratio>=0.8 and ratio <= 1.9)
				...
				if w>25 and h>25 and confidence>0.9 and good_ratio:
					cv2.drawContours(rawImage, [approx], -1, (0, 255, 0), 4)
					cv2.putText(rawImage,"Confidence is %s"%confidence,(20,600),cv2.FONT_HERSHEY_SIMPLEX, 1.5,(0, 255, 0), 2 )


if __name__ == '__main__':
	imagePath = 'E:\Dropbox\Python Codes\onGitHub\Image Processing, targeting, and recognition\Raw_image.jpg'
